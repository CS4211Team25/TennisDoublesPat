...
for line in sys.stdin:
    ...
    if not hitType:
        ...
        if hitType == RET_NORMAL:
            froResults[(*currOrientation, fromPos)][(player, hitType, stroke, result)] += 1
            toResults[(*currOrientation, toPos)][(player, hitType, stroke, result)] += 1
...
for orientation in orientationList:
    print("for orientation:", orientation[:-1], "and position", orientation[4])
    ...
    numStrokes += caseSum
    for case in cases:
        print(f"{case}, {froResults[orientation][case]}")
...
for destination in destinationList:
    print("for destination:", destination[:-1], "and position", destination[4])
    print("We have the following pcases:")
    ...
    numStrokes += caseSum
    for case in cases:
        print(f"{case}, {toResults[destination][case]}")
        if destination[4] != '9':
